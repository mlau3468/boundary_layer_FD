...
function procPanels(pts)
    npan = size(pts,2) - 1

    # collocation pts
    c_pts = zeros(2, npan)
    for i = 1:npan
        c_pts[:,i] .= (pts[:,i] .+ pts[:,i+1])./2
    end

    # panel orientation angles
    thetas = zeros(npan)
    for i = 1:npan
        thetas[i] = -atan(pts[2,i+1]-pts[2,i], pts[1,i+1]-pts[1,i])
    end

    # normal and tangent vectors
    norms = zeros(2, npan)
    tangents = zeros(2, npan)
    for i = 1:npan
        norms[1,i] = sin(thetas[i])
        norms[2,i] = cos(thetas[i])
        tangents[1,i] = cos(thetas[i])
        tangents[2,i] = -sin(thetas[i])
    end
    ...
    return c_pts, thetas, norms, tangents, dists
end

function genNACA(n, nPts=100, show=false)
    nPts = nPts + 1
    c = 1
    ndig = length(n)
    # x coordinates
    ...
    pts = pts'
    if show
        im = plot(pts[1,:], pts[2,:], aspect_ratio=1, line = (1, 2, :blue), marker = (:circle, 3, 0.6, :red));
        display(im)
    end
