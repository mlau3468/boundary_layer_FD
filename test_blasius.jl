...
uinf = 10
rho = 1.225
nu = 0.0002 #kinematic viscosity (m2/s)
mu = nu*rho #absolute or dynamic viscosity (N s/m2)
re = rho*uinf*L/mu
...
bl_thicks = zeros(nx)
for i = 1:nx
    bl_thicks[i] = getBlThick(ys,u[i,:],uinf)
end
...
bl_thicks = zeros(nx)
for i = 1:nx
    bl_thicks[i] = getBlThick(ys[i,:],uvel[i,:],uinf)
end
...
# -------------------------
# Blasius
cf_blasius = 0.33206*4/sqrt(re)
println("Cf=$cf_blasius")